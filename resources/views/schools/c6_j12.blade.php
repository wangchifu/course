        ...
        @foreach($year12 as $v)
            <td>
                @if(file_exists(storage_path('app/public/upload/'.$year->year.'/'.auth()->user()->code.'/c6_'.cht2num($v).'_technology.pdf')))
                    <a href="javascript:open_upload('{{ route('schools.c6_upload',['select_year'=>$year->year,'order'=>'c6','subject'=>'technology','grade'=>cht2num($v)]) }}','新視窗')" class="badge badge-success"><i class="fas fa-check-circle"></i> 再傳</a>
